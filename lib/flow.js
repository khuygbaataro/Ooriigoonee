...
Би чамд өөрийгөө жаахан өөр өнцгөөс харахад тусалъя гэж бодож байна.

Заримдаа бид өөрийнхөө тухай хамгийн энгийн зүйлсийг ч анзаардаггүй юм билээ. Хэт ойрхон байгаа болохоор тэр байх аа.

10 асуулт асууя — нэг минут ч болохгүй. Дараа нь би юу харснаа чамд хэлье. Зөв эсэхийг нь чи өөрөө шийднэ.

Төлбөр ч, бүртгэл ч байхгүй.

Бэлэн бол «1» гэж бичээрэй 👇`;

const QUIZ_INTRO = `Сайхан 🙌

Зөв, буруу хариулт гэж байхгүй шүү. Хамгийн эхэнд санаанд орж ирсэн хариултаа л сонго — тэр нь ихэвчлэн хамгийн үнэн байдаг.

Удаан бодох тусам «байх ёстой» хариулт руу шилждэг юм билээ.`;

const ANALYZING = `Баярлалаа 🙏

Хариултуудыг чинь уншиж байна. Хэсэг хүлээгээрэй — 20-30 секунд болно.`;
...
Хамгийн хүчтэй тал чинь:
${profile.strength}

Өөрөө тэр бүр анзаардаггүй тал:
${profile.blind_spot}`;
}
...
function noticedText(profile) {
  ...
  return `Хариултаас чинь өөр гурван зүйл анзаарагдлаа:

${items}`;
}

const DEEPER_OFFER = `Энэ бол зүгээр л эхний харц.

Чиний хариултад үүнээс хамаагүй их зүйл байна — хүч чинь яг хаана байгаа, стресстэй үед дотор чинь юу болдог, хүмүүстэй яаж холбогддог, ямар орчинд амьсгалж чаддаг вэ гэдэг.

Бүтнээр нь бэлдэх үү?`;

const REPORT_INTRO = `За, бэлдье 📝

Жаахан хүлээгээрэй — энэ удаа арай удна.`;

/** Тайлангийн дараа — энэ мессеж хамгийн чухал. Яриаг ЭНД эхлүүлнэ. */
const AFTER_REPORT = `Ингээд л дуусаж байна.

Одоо нэг зүйл хэлээч: энэ дотроос хамгийн их «аан, үнэн шүү» гэж бодогдсон нь юу байв?

Мөн «энэ бол би биш» гэж бодогдсон зүйл байвал түүнийг нь ч заавал хэлээрэй. Би зөвхөн 10 хариултыг чинь л харсан шүү дээ — алдаж байвал тэр нь бүр сонирхолтой.`;

const DAILY_LIMIT = `Өнөөдөр бид нэлээд ярилаа 🙂

Заримдаа бодол боловсроход хугацаа хэрэгтэй байдаг. Өнөөдөр ярьсан зүйлсээ аваад яваарай — маргааш үргэлжлүүлье. Би энд байна.`;
...
async function runAnalysis(psid, session) {
  ...
  try {
    ...
    await fb.sendQuickReplies(
      psid,
      'Уучлаарай, ямар нэг зүйл буруу болчихлоо 😔 Хариултууд чинь хадгалагдсан шүү — дахин оролдох уу?',
...
export async function deliverFullReport(psid, session, { timeoutMs } = {}) {
  ...
  if (!session.fullReport) {
    ...
    try {
      ...
      await fb.sendQuickReplies(
        psid,
        'Уучлаарай, бэлдэх үед алдаа гарлаа 😔 Дахин оролдоход л хангалттай.',
...
async function generateReply(session, text) {
  // Тестээ өгсөн хүн ҮРГЭЛЖ хөтөчийн гүнзгий хариултыг авна — энэ нь
  // бүтээгдэхүүний гол үнэ цэн тул зардлаар хэмнэх зүйл биш.
  const useClaude = Boolean(session.profile) || config.chatProvider === 'claude';

  const claude = () => (session.profile ? guideReply(session, text) : claudeChat(session, text));
  const openai = () => openaiChat(session, text);
...
async function refreshMemory(psid, session, startedAt = Date.now()) {
  ...
  try {
    ...
    session.memory = await updateMemory(session, { timeoutMs: Math.min(remaining, 18_000) });
...
async function handlePayload(psid, session, payload) {
  ...
  if (payload === 'LATER') {
    ...
    return fb.sendText(
      psid,
      'За, яарах юун 🙂\n\nХүссэн үедээ «үр дүн» гэж бичвэл бэлдээд өгье. Эсвэл зүгээр л ярилцаж болно — юу ч асууж болно шүү.',
    );
  }

  if (payload === 'MY_RESULT') {
    if (!session.profile) {
      return fb.sendQuickReplies(psid, 'Та 10 асуултаа хараахан өгөөгүй байна 🙂', [
        { title: '✨ Эхлэх', payload: 'START_QUIZ' },
      ]);
    }
  ...
  if (payload === 'CHAT') {
    session.state = 'chat';
    await saveSession(psid, session);
    return fb.sendText(psid, 'Сонсож байна 👂 Юу яримаар байна?');
  }
...
async function handleText(psid, session, text) {
  ...
  if (session.state === 'analyzing') {
    if (!analyzingStalled(session)) {
      return fb.sendText(psid, 'Уншиж дуусаж байна… түр хүлээгээрэй ⏳');
    }
...
async function handleAttachments(psid, session, attachments) {
  ...
  if (!image) {
    return fb.sendText(psid, 'Одоогоор зөвхөн үг ойлгодог юм байна 🙈 Бичиж хэлэх үү?');
  }
  ...
  return fb.sendText(
    psid,
    'Зургийг чинь харлаа, гэхдээ би одоогоор зөвхөн үг уншиж чаддаг юм байна 🙈\n\nЮу байсныг бичиж хэлэх үү?',
  );
}
