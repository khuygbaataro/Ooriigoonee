...
import { analyze, deepDive, formatFullReport } from './claude.js';
...
async function runAnalysis(psid, session) {
  ...
  try {
    const person = await fb.getProfile(psid);
    const profile = await analyze(session.answers, { firstName: person?.first_name });

    session.profile = profile;
    session.fullReport = profile.full_report;
    ...
    console.error('[flow] анализ амжилтгүй', err);
    session.state = 'quiz';
    session.questionIndex = TOTAL - 1;
    session.answers = session.answers.slice(0, TOTAL - 1);
    await saveSession(psid, session);
    await fb.sendQuickReplies(
      psid,
      'Уучлаарай, дүн шинжилгээ хийх үед алдаа гарлаа 😔 Дахин оролдох уу?',
...
export async function deliverFullReport(psid, session) {
  ...
  await fb.sendText(psid, '🎉 Төлбөр баталгаажлаа! Таны бүрэн анализ бэлэн боллоо 👇');
  await fb.sendText(psid, formatFullReport(session.profile));
...
/** Чөлөөт чат — төлсөн бол Claude гүнзгий, үгүй бол OpenAI. */
async function handleChat(psid, session, text) {
  ...
  try {
    const reply =
      session.paid && session.profile
        ? await deepDive(session.profile, text)
        : await openaiChat(session, text);
    ...
    console.error('[flow] чат алдаа', err);
...
async function handlePayload(psid, session, payload) {
  ...
  if (payload === 'RETRY_ANALYSIS') {
    if (session.answers.length < TOTAL) {
      return askQuestion(psid, session);
    }
    return runAnalysis(psid, session);
  }
