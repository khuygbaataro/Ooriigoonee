...
export const emptySession = () => ({
  state: 'idle',        // idle | quiz | analyzing | error | chat
  questionIndex: 0,
  answers: [],          // [{ key: 'А', label: '...' }]
...
function migrate(session) {
  ...
  // Гацсан / хуучирсан төлөвүүдийг ярианд буцаана. Бүрэн тайлан гэж
  // байхаа больсон тул түүнийг хүлээж байсан хүмүүс шууд ярианд орно.
  if (['paid_pending', 'report_pending', 'mirror', 'result'].includes(merged.state)) {
    merged.state = merged.profile ? 'chat' : 'idle';
  }
  delete merged.fullReport;
