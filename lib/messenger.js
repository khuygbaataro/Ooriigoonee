import crypto from 'node:crypto';
import { config } from './config.js';

/** Facebook Send API-тай харилцах бүх зүйл энд. */

/** Claude-ийн зургийн хязгаар 5MB — бага зэрэг нөөцтэй авав. */
const MAX_IMAGE_BYTES = 4 * 1024 * 1024;
const IMAGE_TYPES = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];

const GRAPH = () => `https://graph.facebook.com/${config.graphVersion}`;
const TEXT_LIMIT = 1900;      // FB хязгаар 2000 — жаахан нөөцтэй
const TITLE_LIMIT = 20;       // quick reply / товчны гарчиг

async function call(path, body, method = 'POST') {
  if (!config.pageAccessToken) {
    console.error('[messenger] PAGE_ACCESS_TOKEN алга байна');
    return null;
  }
  const res = await fetch(
    `${GRAPH()}/${path}?access_token=${encodeURIComponent(config.pageAccessToken)}`,
    {
      method,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    },
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    console.error('[messenger] алдаа', res.status, JSON.stringify(data));
    return null;
  }
  return data;
}

const send = (psid, message) =>
  call('me/messages', {
    messaging_type: 'RESPONSE',
    recipient: { id: psid },
    message,
  });

/** Урт текстийг мөрөөр таслаж 1900 тэмдэгтийн хэсгүүд болгоно. */
export function chunk(text, limit = TEXT_LIMIT) {
  const parts = [];
  let buffer = '';
  for (const line of String(text).split('\n')) {
    if (buffer.length + line.length + 1 > limit) {
      if (buffer) parts.push(buffer.trimEnd());
      buffer = '';
      // Ганц мөр өөрөө хэтэрхий урт бол хүчээр хуваана
      let rest = line;
      while (rest.length > limit) {
        parts.push(rest.slice(0, limit));
        rest = rest.slice(limit);
      }
      buffer = rest + '\n';
    } else {
      buffer += line + '\n';
    }
  }
  if (buffer.trim()) parts.push(buffer.trimEnd());
  return parts.length ? parts : [''];
}

export async function sendText(psid, text) {
  for (const part of chunk(text)) {
    await send(psid, { text: part });
  }
}

export async function senderAction(psid, action = 'typing_on') {
  return call('me/messages', { recipient: { id: psid }, sender_action: action });
}

export async function sendQuickReplies(psid, text, replies) {
  const parts = chunk(text);
  const last = parts.pop();
  for (const part of parts) await send(psid, { text: part });
  return send(psid, {
    text: last,
    quick_replies: replies.slice(0, 13).map((r) => ({
      content_type: 'text',
      title: r.title.slice(0, TITLE_LIMIT),
      payload: r.payload,
    })),
  });
}

/**
 * Товчтой мессеж. buttons = [{type:'postback',title,payload}] эсвэл
 * [{type:'web_url',title,url}] — хамгийн ихдээ 3.
 */
export async function sendButtons(psid, text, buttons) {
  return send(psid, {
    attachment: {
      type: 'template',
      payload: {
        template_type: 'button',
        text: text.slice(0, 640),
        buttons: buttons.slice(0, 3).map((b) => ({
          ...b,
          title: b.title.slice(0, TITLE_LIMIT),
        })),
      },
    },
  });
}

/**
 * Хэрэглэгчийн явуулсан зургийг татаж base64 болгоно (Claude-д өгөхөд бэлэн).
 * sha256 нь нэг баримтыг дахин дахин ашиглахаас хамгаалахад хэрэгтэй.
 */
export async function downloadImage(url) {
  const res = await fetch(url, { redirect: 'follow' });
  if (!res.ok) throw new Error(`Зураг татаж чадсангүй: HTTP ${res.status}`);

  const mediaType = (res.headers.get('content-type') ?? '').split(';')[0].trim().toLowerCase();
  if (!IMAGE_TYPES.includes(mediaType)) {
    throw new Error(`Дэмжигдэхгүй файлын төрөл: ${mediaType || 'үл мэдэгдэх'}`);
  }

  const declared = Number(res.headers.get('content-length') ?? 0);
  if (declared > MAX_IMAGE_BYTES) throw new Error('Зураг хэт том байна');

  const buffer = Buffer.from(await res.arrayBuffer());
  if (buffer.byteLength > MAX_IMAGE_BYTES) throw new Error('Зураг хэт том байна');

  return {
    mediaType,
    data: buffer.toString('base64'),
    bytes: buffer.byteLength,
    sha256: crypto.createHash('sha256').update(buffer).digest('hex'),
  };
}

/**
 * PAGE_ACCESS_TOKEN хүчинтэй эсэхийг шалгах — /api/health?deep=1 дээр ашиглана.
 * Алдаа гарвал Facebook-ийн ЖИНХЭНЭ мессежийг шидэж, шалтгааныг харуулна.
 */
export async function pingPage() {
  if (!config.pageAccessToken) throw new Error('PAGE_ACCESS_TOKEN тохируулаагүй байна');
  const res = await fetch(
    `${GRAPH()}/me?fields=id,name&access_token=${encodeURIComponent(config.pageAccessToken)}`,
  );
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(`Facebook ${res.status}: ${data?.error?.message ?? JSON.stringify(data)}`);
  }
  return { page: data.name ?? null, pageId: data.id ?? null };
}

/** Хэрэглэгчийн нэрийг авах (мэндчилгээнд ашиглана). Алдаа гарвал null. */
export async function getProfile(psid) {
  if (!config.pageAccessToken) return null;
  try {
    const res = await fetch(
      `${GRAPH()}/${psid}?fields=first_name,last_name&access_token=${encodeURIComponent(config.pageAccessToken)}`,
    );
    if (!res.ok) return null;
    return await res.json();
  } catch {
    return null;
  }
}

/**
 * Messenger-ийн "Get Started" товч болон мэндчилгээг суулгана.
 * Deploy хийсний дараа /api/setup?secret=... гэж нэг удаа дуудна.
 *
 * ⚠️ ДООД ЦЭС ЗОРИУД БАЙХГҮЙ.
 *    Хүнтэй ярьж байхад доор нь «цэс» өлгөөтэй байх нь энэ бол аппликейшн
 *    гэдгийг байнга сануулна. Цэсний бүх үйлдэл бичихэд ажилладаг:
 *      «1» → эхлэх · «үр дүн» → тайлан · «устгах» → өгөгдөл устгах
 *    Тиймээс цэс нь дэлгэц иддэгээс өөр юу ч нэмэхгүй.
 */
export async function installMenu() {
  // Өмнө нь суулгасан цэсийг ЗААВАЛ устгана — POST дээр талбарыг нь орхиход
  // Facebook хуучин утгыг хэвээр үлдээдэг, өөрөө алга болохгүй.
  await call('me/messenger_profile', { fields: ['persistent_menu'] }, 'DELETE').catch(() => null);

  return call('me/messenger_profile', {
    get_started: { payload: 'GET_STARTED' },
    greeting: [
      {
        locale: 'default',
        // ⚠️ «Үнэгүй» гэж зориуд хэлэхгүй — мөнгийг яриан дунд оруулахгүйн тулд.
        text: 'Өөрийгөө хэр сайн мэддэг вэ? 👀 10 асуулт, нэг минут.',
      },
    ],
  });
}
