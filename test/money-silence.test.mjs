...
test('lib/flow.js доторх илгээгдэх текстэд мөнгөний үг байхгүй', () => {
  ...
  /**
   * ⚠️ ҮЛ ХАМААРАХ ЗҮЙЛ БАЙХГҮЙ — «үнэгүй», «төлбөргүй» ч гэсэн ХОРИОТОЙ.
   *
   * Мөнгөний тухай ярихгүй гэж хэлэх нь ч гэсэн мөнгийг яриан дунд оруулна.
   * Хүн шинэ хүнтэй танилцахдаа «энэ үнэгүй шүү» гэж эхэлдэггүй — тэр нь
   * зарж байгаа хүний хэллэг. Хэн нэгэн асуувал lib/gift.js хариулна.
   */
  const moneyWords = [
    'мөнгө', 'төлбөр', 'төлөх', 'данс', 'хандив', 'төгрөг', '₮',
    'үнэтэй', 'үнэгүй', 'төлбөргүй',
  ];
  ...
  assert.deepEqual(
    offenders,
    [],
    'flow.js-ийн илгээгдэх текстэд мөнгөний үг орж ирлээ — «үнэгүй» гэж хэлэх нь ч ' +
      'мөнгийг яриан дунд оруулна. Бүх мөнгөний текст lib/gift.js дотор байх ёстой.',
  );
});

test('Messenger-ийн мэндчилгээ ч мөнгө дурдахгүй', () => {
  // Мэндчилгээ нь хүн ярианы товчийг дарахаас ӨМНӨ харагдана — хамгийн
  // эхний сэтгэгдэл тул тэнд ч «үнэгүй» гэсэн үг байх ёсгүй.
  // Тайлбар мөрийг хасна — тэнд дүрмээ ТАЙЛБАРЛАХ нь зөвшөөрөгдөнө,
  // зөвхөн БОДИТООР илгээгдэх текстийг шалгана.
  const src = read('lib/messenger.js')
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .replace(/^\s*\/\/.*$/gm, '');

  const at = src.indexOf('greeting:');
  assert.ok(at > 0, 'мэндчилгээ олдсонгүй');

  const block = src.slice(at, at + 400);
  for (const w of ['үнэгүй', 'төлбөр', 'мөнгө', '₮']) {
    assert.ok(!block.toLowerCase().includes(w), `мэндчилгээнд «${w}» байх ЁСГҮЙ`);
  }
});
